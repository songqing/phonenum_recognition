...
IMAGEDATA **imagedataTempDouble=NULL;//动态分配存储原图片的像素信息的二维数组
...
int readBmp(const char* filename)
{
	...
	if(fpi != NULL){
		...
		//temp imagedata array for 40 * 28
		imagedataTempDouble= (IMAGEDATA**)malloc(40*28*sizeof(IMAGEDATA));
		for(i=0;i<28;i++)
		  imagedataTempDouble[i]=(IMAGEDATA*)malloc(40*sizeof(IMAGEDATA));
...
int createDoubleNum(int curPic)
{
	FILE* fpw, *fout2,*fpPos,*fplarge;
	int i,j;
	int phonenumTemp=curPic;
	...
	int iNum;
	const char* filecreate="numpic/num00/00_0000.bmp";
	char filecreateTemp[]="numpic/num00/00_0000.bmp";
//	int numvalue=0,numpos=0;
	int numvalue_old=0,numvalue_new=0,numpos_old=0,numpos_new=0;
	int linesmall,rowsmall;
	fscanf(fpPos,"%d\t%d\t",&numvalue_old,&numpos_old);
	for(iNum=1;iNum<11;iNum++)  //get the 11 num's real value and pos
	{
		fscanf(fpPos,"%d\t%d\t",&numvalue_new,&numpos_new);

		//保存bmp图片
		i=0;
		filecreateTemp[15]='_';
		filecreateTemp[18]=filecreateTemp[19]=filecreateTemp[16]=filecreateTemp[17]='0';
		phonenumTemp=countNumDouble[numvalue_old*10+numvalue_new]++;
		while(phonenumTemp>0)
		{
			filecreateTemp[19-i++]=phonenumTemp%10 + '0';
			phonenumTemp/=10;
		}
		filecreateTemp[10]=filecreateTemp[13]=numvalue_old+'0';
		filecreateTemp[11]=filecreateTemp[14]=numvalue_new+'0';
		filecreate=filecreateTemp;
		if((fpw=fopen(filecreate,"wb"))==NULL)  //file increase by 1
		{
			cout<<"create the bmp file error!"<<endl;
		...
		for(i=7;i<=23;i++)
		  fprintf(fplist,"%c",filecreateTemp[i]);
		fprintf(fplist," %d%d\n",filecreateTemp[10]-'0',filecreateTemp[11]-'0');
		...
		{
			j=8;
			for(rowsmall=numpos_new-16;rowsmall<=numpos_new+7;rowsmall++)
			{
				if(rowsmall<0)
				  continue;
				imagedataTempDouble[i][j]=imagedata[linesmall][rowsmall];
				j++;
			}
			i++;
		}
		...
		for(i=27;i>=0;i--)
		  fwrite(imagedataTempDouble[i],sizeof(char),40,fpw);

		fclose(fpw);
		numvalue_old=numvalue_new;
		numpos_old=numpos_new;
	}
