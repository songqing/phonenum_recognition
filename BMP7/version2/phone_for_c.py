...
#put into the list OR update, limit operation
			...
			if(nummax==0 and prediction[0][nummax]> 0.998 and finallist[-1]!=1):
				finallist.append(nummax)
				ratelist.append(prediction[0][nummax])
				poslist.append(iternum)
			elif(finallist[-1]==1 and nummax==0 and ratelist[-1]<0.999 and prediction[0][nummax]> 0.998):
				finallist[-1]=nummax
				ratelist[-1]=prediction[0][nummax]
				poslist[-1]=iternum
			elif(nummax==finallist[-1] and finallist[-1]==finallist[-2] and prediction[0][nummax]> ratelist[-1]):
				finallist[-1]=nummax
				ratelist[-1]=prediction[0][nummax]
				poslist[-1]=iternum
			...
			if(len(finallist)==1):#add not update
				...
				if(ratelist[-1]> 0.999 and len(finallist) < 11):# last rate is too high, add
					...
					if(nummax==finallist[-1] and finallist[-1]== finallist[-2]):
						...
						ratelist.append(prediction[0][nummax])
						poslist.append(iternum)
				elif(ratelist[-1]> 0.998 and prediction[0][nummax]>0.999 and nummax!=1 and len(finallist) < 11):# cunrent rate is too high, add
					finallist.append(nummax)
					ratelist.append(prediction[0][nummax])
					poslist.append(iternum)
				elif(finallist[-1]==1 and nummax==6 and ratelist[-1]< 0.998 and prediction[0][nummax]>0.99):# 1 afer 6, update
					if(iternum-poslist[-2]<14):
						finallist[-1]=nummax
						ratelist[-1]=prediction[0][nummax]
						poslist[-1]=iternum
						...
						if(nummax==1):
							if(finallist[-1]==0):
								if(prediction[0][nummax] > 0.9999):
									...
									ratelist[-1]=prediction[0][nummax]
									poslist[-1]=iternum
								elif(finallist[-2]==1 and prediction[0][nummax]>0.999 and ratelist[-1]<0.993):
									finallist[-1]=nummax
									ratelist[-1]=prediction[0][nummax]
									poslist[-1]=iternum
...
#direct put into the list
			...
			if(len(finallist) <= 2):
				...
				if(nummax==finallist[-1] and finallist[-1]== finallist[-2]):
					...
					if(nummax==1):
						if(finallist[-1]==0):
							if(iternum-poslist[-1]>6 and prediction[0][nummax]>0.996):
								finallist.append(nummax)
								ratelist.append(prediction[0][nummax])
								poslist.append(iternum)
